...
name: vibe-research
description: |
  Vibe Coding 1단계: 심층 리서치 (Enhanced). 코드 작성 없이 코드베이스를 깊이 분석해 .vibe/NNN_topic.md 문서를 생성한다.
  다음 상황에서 사용:
  (1) 새 기능 구현 전 코드베이스 분석이 필요할 때
  (2) 버그 원인을 심층 조사할 때
  (3) 리팩토링 범위를 파악할 때
  (4) "vibe research", "리서치", "분석해줘" 등의 요청 시
argument-hint: '"<분석할 기능 또는 주제>" [--deep] [--patterns] [--graph]'
---

# Vibe Research (Enhanced)

Vibe Coding 방법론의 **1단계: 심층 리서치** - 고급 코드 분석 기능 강화 버전.
...
🔧 분석 모드:
  ...
  - [ ] --deep: 성능 분석, 메모리 프로파일링 포함
  - [ ] --patterns: 코드 패턴 및 안티패턴 감지
  - [ ] --graph: 의존성 그래프 생성
```
