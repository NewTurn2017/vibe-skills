...
```bash
# 승인 상태 자동 체크
grep "개발자 최종 승인.*✓" .vibe/*_plan.md

# 리스크 레벨 확인
grep "리스크 레벨:.*Critical" .vibe/*_plan.md && echo "⚠️ Critical Risk!"
```
...
.vibe/NNN_topic_plan.md 파일을 검토하고
...
가장 최근 plan.md (또는 `--plan` 지정 파일)를 읽고 선언:
```
🚀 구현을 시작합니다.

📋 Plan: .vibe/NNN_topic_plan.md
📊 구현 메타데이터:
- 변경 파일: N개
- 예상 시간: X hours
- 리스크 레벨: Medium
...
function emergency_rollback() {
  ...
  # 2. 마지막 안정 체크포인트로 복원
  LAST_STABLE=$(git stash list | grep "vibe-checkpoint" | head -1 | cut -d: -f1)
